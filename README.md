...
- ✅ **New OBS Browser Source:** `inference-results.html` displays model inference results with stylized names, bold percentages, and exact model details
...
### Inference Results Display (NEW)

A new OBS Browser Source (`inference-results.html`) displays the most recent inference results for each provider with:

- **Title:** "AI Tells Time" in Futura font
- **Last Guess Section:** Shows each provider's model name, exact model identifier, accuracy percentage with visual bar, and ✅/❌ indicator
- **Provider Details Section:** Lists provider names and their corresponding model names

**File Location:** `obs-assets/inference-results.html`
**JSON Data Source:** `obs-assets/last-guess.json` (auto-generated by the main loop)
**Dimensions:** 425px × 1080px (same as charts.html)
